...
export default function ImportUsersScreen() {
  ...
  const processWorkbook = (workbook: XLSX.WorkBook) => {
    ...
    // Walidacja ról
    const validRoles = ["admin", "zarzad", "project_manager", "bauleiter", "worker"];
  ...
  const importUsers = async () => {
    ...
    try {
      ...
      if (createdUsers.length > 0) {
        Alert.alert(
          t("common.success"),
          t("users.import.success", { count: createdUsers.length }) +
            "\n\n" +
            t("users.import.temp_password_info"),
