 ...
 * - 1 useQuery: materials (raw warehouse_materials, sortowane po pozycji)
 * - 3 useMutation: saveMatItem, deleteMatItem, importBatch
 *
 * Klucz cache: warehouseKeys.materials() (z useProjectOrders) -- wspolny
 * z lista materialow w zamowieniach. Mutations invalidate ten klucz.
 */
...
export function useWarehouseMaterials(
  ...
  const saveMatMutation = useMutation({
    mutationFn: async ({ payload, editing }: { payload: Record<string, unknown>; editing: MaterialItem | null }) => {
      if (editing) {
        const { error } = await supabaseAdmin.from("warehouse_materials").update(payload).eq("id", editing.id);
        if (error) throw error;
      } else {
        const { error } = await supabaseAdmin.from("warehouse_materials").insert({ ...payload, created_by: profileId || null });
        if (error) throw error;
      }
    },
    onSuccess: () => {
      setShowMatModal(false);
      qc.invalidateQueries({ queryKey: warehouseKeys.materials() });
    },
    onError: (e) => {
  ...
  const deleteMatMutation = useMutation({
    ...
    onError: (e) => console.error("Error deleting material:", e),
  });
  ...
  const importBatchMutation = useMutation({
    mutationFn: async (rows: ExcelCell[][]) => {
      ...
      return { inserted, total: toInsert.length };
    },
    ...
    onError: (e) => {
      ...
      console.error("Mat import error:", e);
    },
  });

  // ── Action wrappers ──

  const loadMaterials = async () => { await materialsQuery.refetch(); };
  ...
  const saveMatItem = () => {
    ...
    saveMatMutation.mutate({ payload, editing: editingMat });
  };
  ...
  return {
    // Data
    materials,
    matLoading: materialsQuery.isPending,
    // Search
    matSearch, setMatSearch,
    // Import/Export
    matImportLoading: importBatchMutation.isPending,
    handleMatImport, handleMatExport,
    // CRUD modal
    showMatModal, setShowMatModal, editingMat, matForm, setMatForm,
    matSaving: saveMatMutation.isPending,
